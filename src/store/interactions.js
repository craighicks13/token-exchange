...
export const susbscribeToEvents = async (exchange, dispatch) => {
...
export const depositToken = async (dispatch, exchange, web3, token, amount, account) => {
  ...
  await exchange.methods.depositToken(token.options.address, amount).send({ from: account })
    ...
    .on('error',(error) => {
      console.error(error)
      window.alert(`There was such an error!`)
    })
  /*
...
export const withdrawToken = async (dispatch, exchange, web3, token, amount, account) => {
